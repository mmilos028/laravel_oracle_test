...
class MemberPdfController extends Controller
{
    ...
    public function memberCardCreatePdfHorizontalLayout(Request $request)
    {
...
CARD;
            ...
            $pdf_title = "MemberCard-WIDE-{$from_serial}-{$to_serial}.pdf";
            TCPDF::Output($pdf_title, 'D');            
        //}
    }
    
    public function memberCardCreatePdf(Request $request)
    {
        $backoffice_session_id = $request->get('backoffice_session_id', null);
        $backoffice_username = $request->get('backoffice_username', null);
        $from_serial = $request->get('serial_number_start', null);
...
CARD;
            ...
            TCPDF::lastPage();
            $pdf_title = "MemberCard-{$from_serial}-{$to_serial}.pdf";
            TCPDF::Output($pdf_title, 'D');
        //}
    }
    
    public function memberCardCreateQrPdf(Request $request)
    {
        $backoffice_session_id = $request->get('backoffice_session_id', null);
        $backoffice_username = $request->get('backoffice_username', null);
        $from_serial = $request->get('serial_number_start', null);
        $to_serial = $request->get('serial_number_end', null);
        $gameClientUrl = $request->get('qr_code_url', null);
        ...
        foreach ($cards_data as $data) {
            ...
            if (isset($data['username']) && $data['username'] && isset($data['password']) && $data['password']) {
                //$barcode_size = 22;
                //$card_name = "Prepaid Card";
                $url_address = "{$gameClientUrl}?barcode=" . $data['prepaid_code'];
                $qr_code = TCPDF::serializeTCPDFtagParameters(array($url_address, 'QRCODE,H', '', '', 25, 25, $style, 'N'));
                $limit_per_page = 9;
                $card_member_addon = <<<CARD_MEMEBER_ADDON
			<tr><td colspan="3" style="font-size:5px;"></td></tr>
			<tr>
...
CARD_MEMEBER_ADDON;
		...
		<table style="height:85.60mm;background:#FFF;width:54mm;margin:10mm;padding:0;border:1px dashed #666" cellpadding="1" border="0">
			...
			<tr>
				<td style="width:100%;font-size:1mm" colspan="3"><tcpdf method="write2DBarcode" params="{$qr_code}"/></td>
			</tr>
		</table>
CARD;
			if ($j==1) {
        ...
        TCPDF::lastPage();
        //$pdf_title = "PPCard-" . date("Ymd-His") . ".pdf";
        $pdf_title = "MemberCard-QR-{$from_serial}-{$to_serial}.pdf";
        TCPDF::Output($pdf_title, 'D');
    }
    
    public function memberCardCreateQrPdfHorizontalLayout(Request $request)
    {
        $backoffice_session_id = $request->get('backoffice_session_id', null);
        $backoffice_username = $request->get('backoffice_username', null);
        $from_serial = $request->get('serial_number_start', null);
        ...
        //if ($request->isMethod('POST')) {            
            ...
            foreach ($cards_data as $data) {
                ...
                if (isset($data['username']) && $data['username'] && isset($data['password']) && $data['password']) {
                    //$barcode_size = 12;
                    $url_address = "{$gameClientUrl}?barcode=" . $data['prepaid_code'];
                    //$qr_code = $pdf->serializeTCPDFtagParameters(array($url_address, 'QRCODE,H', '', '', 0, $barcode_size, 0.3, $style, 'N'));
                    $qr_code = TCPDF::serializeTCPDFtagParameters(array($url_address, 'QRCODE,H', '', '', 35, 35, $style, 'N'));
                    $card_name = "Prepaid Card";
                    $limit_per_page = 6;
                    $card_member_addon = <<<CARD_MEMEBER_ADDON
                    <tr>
...
CARD_MEMEBER_ADDON;
                ...
                <table style="height:53.98mm;background:#FFF;width:85.60mm;margin:10mm;padding:0;border:1px dashed #666" cellpadding="1" border="0">
                    ...
                    <tr>
                        <td style="font-size:1mm" colspan="3"><tcpdf method="write2DBarcode" params="{$qr_code}"/></td>
                    </tr>
                    <tr>
                        <td style="font-size:3mm;" colspan="3">&nbsp;Voucher code: <span style="font-weight:bold;">{$code_to_show}</span></td>
                    </tr>
...
CARD;
            ...
            TCPDF::lastPage();
            //$pdf_title = "PPCard-" . date("Ymd-His") . ".pdf";
            $pdf_title = "MemberCard-QR-Wide-{$from_serial}-{$to_serial}.pdf";
            TCPDF::Output($pdf_title, 'D');
